...
- **Environment Templates**: Added `DATA_DIR` to `template.dev.env` (`data/dev`) and `template.prod.env` (`data/prd`) to ensure strict isolation between stages.
- **Comprehensive Documentation**: Added ADR-002, created `storage-client.md`, and updated all relevant technical docs to reflect the hybrid storage strategy and environment-specific paths.
