...
To ensure absolute data isolation within a shared Prefect cluster, we use environment-specific Docker images.

1.  **Build Phase**: Configuration (DB host, port, credentials) is baked into the image using `--build-arg`.
2.  **Registration Phase**: Deployments are registered with a suffix (e.g., `EOD-Saver/dev`) pointing to the respective image.
